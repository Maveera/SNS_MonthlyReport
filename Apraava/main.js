...
function renderCharts() {
  ...
  const alertRows = parseChartPairs(getValue("alertCsv"));
  const alertCtx = document.getElementById("alertSeverityChart");
  if (alertCtx) {
    if (alertChart) alertChart.destroy();
    alertChart = new Chart(alertCtx, {
      type: "bar",
      data: {
        labels: alertRows.map((r) => r.label),
        datasets: [
          {
            label: "Alerts",
            data: alertRows.map((r) => r.value),
            backgroundColor: ["#be2f2f", "#cf9409", "#1e4f9a"]
          }
        ]
      },
  ...
  const tpfpRows = parseCsvRows(getValue("tpfpCsv"), ["type", "high", "medium", "low"]);
  const tpfpCtx = document.getElementById("tpfpChart");
  if (tpfpCtx) {
    if (tpfpChart) tpfpChart.destroy();
    tpfpChart = new Chart(tpfpCtx, {
      type: "bar",
      data: {
        labels: ["High", "Medium", "Low"],
        datasets: tpfpRows.map((row, idx) => ({
          label: row.type,
          data: [Number(row.high) || 0, Number(row.medium) || 0, Number(row.low) || 0],
          backgroundColor: idx === 0 ? "#1e4f9a" : "#be2f2f"
        }))
      },
      options: { responsive: true, maintainAspectRatio: false }
    });
...
function applyData() {
  ...
  const fwEl = document.getElementById("firewallTicketsPreview");
  if (fwEl) fwEl.innerHTML = formatRichTextHTML(getValue("firewallTicketsText"));
  ...
  const highestRows = parseCsvRows(getValue("highestEpsCsv"), ["device", "eventType", "eventName", "matched"]);
  renderTableRows(
    "highestEpsTableBody",
    highestRows,
    (row) =>
      `<td>${row.sno}</td><td>${row.device}</td><td>${row.eventType}</td><td>${row.eventName}</td><td>${row.matched}</td>`
  );
...
async function exportPptx() {
  ...
  if (key === "blupine") {
    ...
    slide = pptx.addSlide();
    addPptxTitleBar(slide, "Highest EPS Consuming Events");
    const hiRows = [
      [
        { text: "S.No", options: { bold: true, fill: { color: "F3F6FB" } } },
        { text: "Device", options: { bold: true, fill: { color: "F3F6FB" } } },
        { text: "Event Type", options: { bold: true, fill: { color: "F3F6FB" } } },
        { text: "Event Name", options: { bold: true, fill: { color: "F3F6FB" } } },
        { text: "Matched", options: { bold: true, fill: { color: "F3F6FB" } } }
      ],
      ...hi.map((r) => [String(r.sno), r.device, r.eventType, r.eventName, r.matched])
    ];
    slide.addTable(hiRows, { x: 0.35, y: 0.6, w: 9.3, fontSize: 8, colW: [0.4, 1.5, 1.8, 2.5, 2.1] });
    addPptxFooter(slide);

    slide = pptx.addSlide();
    addPptxTitleBar(slide, "Firewall Support Tickets");
    slide.addText(parseRichTextPptx(getValue("firewallTicketsText")), { x: 0.5, y: 0.65, w: 9, h: 3, fontSize: 10 });
    addPptxFooter(slide);
  }

  const alertImg = canvasToPptxData(document.getElementById("alertSeverityChart"));
  if (alertImg) {
    slide = pptx.addSlide();
    addPptxTitleBar(slide, "Total Alerts Triggered in FortiSIEM");
    slide.addImage({ data: alertImg, x: 0.8, y: 0.65, w: 8.4, h: 4.2 });
    addPptxFooter(slide);
  }

  const tpfpImg = canvasToPptxData(document.getElementById("tpfpChart"));
  if (tpfpImg) {
    slide = pptx.addSlide();
    addPptxTitleBar(slide, "True Positive vs False Positive");
    slide.addImage({ data: tpfpImg, x: 0.8, y: 0.65, w: 8.4, h: 4.2 });
    addPptxFooter(slide);
  }
...
if (trendCsvInput) {
  trendCsvInput.addEventListener("change", (e) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        document.getElementById("trendCsv").value = event.target.result;
      };
...
const potIncCsvInput = document.getElementById("potIncidentsCsvFile");
if (potIncCsvInput) {
  potIncCsvInput.addEventListener("change", (e) => {
    const file = e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        document.getElementById("potIncidentsCsv").value = event.target.result;
      };
      reader.readAsText(file);
    }
  });
...
const epsTableCsvInput = document.getElementById("epsTableCsvFile");
if (epsTableCsvInput) {
  epsTableCsvInput.addEventListener("change", (e) => {
    const file = e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        document.getElementById("epsTableCsv").value = event.target.result;
      };
      reader.readAsText(file);
    }
  });
...
const deviceCsvInput = document.getElementById("deviceCsvFile");
if (deviceCsvInput) {
  deviceCsvInput.addEventListener("change", (e) => {
    const file = e.e.target ? e.target.files[0] : (e.dataTransfer ? e.dataTransfer.files[0] : null) || e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        document.getElementById("deviceCsv").value = event.target.result;
      };
      reader.readAsText(file);
    }
  });
...
const riskCsvInput = document.getElementById("riskCsvFile");
if (riskCsvInput) {
  riskCsvInput.addEventListener("change", (e) => {
    const file = e.e?.target ? e.target.files[0] : (e.dataTransfer ? e.dataTransfer.files[0] : null) || e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        document.getElementById("riskCsv").value = event.target.result;
      };
      reader.readAsText(file);
    }
  });
...
const invCsvInput = document.getElementById("inventoryCsvFile");
if (invCsvInput) {
  invCsvInput.addEventListener("change", (e) => {
    const file = e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        document.getElementById("inventoryCsv").value = event.target.result;
      };
      reader.readAsText(file);
    }
  });
